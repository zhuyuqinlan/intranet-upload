# 局域网文件中心 v2.0

🌸 基于 Vue 3 + NestJS 的现代化局域网文件共享系统

## 技术栈

### 前端
- **框架**: Vue 3 (Composition API)
- **语言**: TypeScript
- **UI 组件库**: Naive UI
- **状态管理**: Pinia
- **构建工具**: Vite
- **样式**: SCSS

### 后端
- **框架**: NestJS
- **语言**: TypeScript
- **文件上传**: Multer (分片上传)
- **数据存储**: JSON 文件

## 功能特性

- 📁 **文件夹管理**: 支持创建文件夹、树形目录结构、面包屑导航
- 🚀 **分片上传**: 大文件分片上传，支持断点续传
- 🔄 **批量操作**: 支持批量删除、多选文件
- 🔍 **搜索过滤**: 快速查找文件
- 🎨 **二次元风格**: 粉色主题，圆角设计，流畅动画
- 💾 **离线部署**: 无外部 CDN 依赖，适合内网环境

## 快速开始

### 环境要求

- Node.js >= 18.0.0
- pnpm >= 8.0.0

### 安装

```bash
# 安装 pnpm（如果尚未安装）
npm install -g pnpm

# 安装依赖
pnpm install
```

### 开发

```bash
# 同时启动前后端开发服务器
pnpm dev

# 或者分别启动
pnpm --filter server dev  # 后端: http://localhost:9000
pnpm --filter web dev     # 前端: http://localhost:5173
```

### 构建

```bash
# 构建前后端
pnpm build

# 单独构建
pnpm build:server
pnpm build:web
```

### 生产运行

```bash
# 构建后启动
pnpm build
pnpm start
```

访问: http://localhost:9000

## 项目结构

```
intranet-file-center/
├── server/                    # NestJS 后端
│   ├── src/
│   │   ├── main.ts           # 应用入口
│   │   ├── app.module.ts     # 根模块
│   │   ├── config/           # 配置
│   │   ├── common/           # 通用模块（响应、异常拦截器）
│   │   ├── upload/           # 分片上传模块
│   │   └── files/            # 文件管理模块
│   └── package.json
│
├── web/                       # Vue3 前端
│   ├── src/
│   │   ├── main.ts           # 入口
│   │   ├── App.vue           # 根组件
│   │   ├── api/              # API 封装
│   │   ├── composables/      # 组合式函数
│   │   ├── stores/           # Pinia 状态管理
│   │   ├── views/            # 页面组件
│   │   └── assets/styles/    # 样式文件
│   └── package.json
│
├── data/                      # 数据目录（统一存放）
│   ├── uploads/              # 用户上传文件
│   ├── temp/                 # 分片上传临时目录
│   └── file-info.json        # 文件元数据
│
├── package.json              # Monorepo 根配置
├── pnpm-workspace.yaml
├── Dockerfile
└── ecosystem.config.js       # PM2 配置
```

## 配置

### 环境变量

创建 `.env` 文件：

```env
# 服务端口
PORT=9000

# 目录配置（统一在 data 目录下）
UPLOAD_DIR=./data/uploads
TEMP_DIR=./data/temp
FILE_INFO_PATH=./data/file-info.json

# CORS 配置
CORS_ORIGIN=*
```

## API 接口

### 上传接口

| 方法 | 路径 | 功能 |
|------|------|------|
| POST | /api/upload/init | 初始化分片上传 |
| POST | /api/upload/chunk | 上传分片 |
| POST | /api/upload/complete | 完成上传 |

### 文件管理接口

| 方法 | 路径 | 功能 |
|------|------|------|
| GET | /api/files | 获取文件列表 |
| DELETE | /api/files/batch | 批量删除 |
| DELETE | /api/files/:id | 删除单个文件 |
| GET | /api/download/:id | 下载文件 |
| POST | /api/folders | 创建文件夹 |
| GET | /api/stats | 获取统计信息 |

## 内网部署

### Docker 部署

```bash
# 构建镜像
docker build -t file-center .

# 运行容器
docker run -d -p 9000:9000 -v $(pwd)/data:/app/data file-center
```

### PM2 部署

```bash
# 安装 PM2
npm install -g pm2

# 启动服务
pm2 start ecosystem.config.js

# 开机自启
pm2 startup
pm2 save
```

## 注意事项

- ⚠️ 删除操作不可恢复，请谨慎操作
- ⚠️ 大文件上传需要稳定的网络环境
- ⚠️ 定期清理 `data/temp/` 目录
- ⚠️ 备份 `data/` 目录以防止数据丢失

## License

ISC
